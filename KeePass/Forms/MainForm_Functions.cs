...
namespace KeePass.Forms
{
    public partial class MainForm : Form
    {
        ...
        public DocumentManagerEx DocumentManager
        {
            get
            {
                return m_docMgr;
        ...
        public PwDatabase ActiveDatabase
        {
            get
            {
                return m_docMgr.ActiveDatabase;
            }
        }
        ...
        public ContextMenuStrip EntryContextMenu
        {
            get
            {
                return m_ctxPwList;
        ...
        public ToolStripProgressBar MainProgressBar
        {
            get
            {
                return m_statusPartProgress;
            }
        }
        public NotifyIcon MainNotifyIcon
        {
            get
            {
                return m_ntfTray.NotifyIcon;
            }
        }

        public MruList FileMruList
        {
            get
            {
                return m_mruList;
        ...
        private void CleanUpEx()
        {
            ...
            if (m_fontBoldUI != null) {
                m_fontBoldUI.Dispose();
                m_fontBoldUI = null;
            }
            if (m_fontBoldTree != null) {
                m_fontBoldTree.Dispose();
                m_fontBoldTree = null;
            }
            if (m_fontItalicTree != null) {
                m_fontItalicTree.Dispose();
        ...
        private void UpdateUIState(bool bSetModified, Control cOptFocus)
        {
            ...
            if (s.FileLocked) {
                ...
                if (icoDisposable != null)
                    icoDisposable.Dispose();
        ...
        private void UpdateGroupList(PwGroup pgNewSelected)
        {
            ...
            if (pwDb.RootGroup != null) {
                ...
                if (m_fontBoldTree != null)
                    tnRoot.NodeFont = m_fontBoldTree;
            ...
            PwGroup pgFind, ref TreeNode tnFound)
        {
            ...
            foreach (PwGroup pg in pgContainer.Groups) {
                ...
                if (tn.Nodes.Count > 0) {
                    if (tn.IsExpanded && !pg.IsExpanded)
                        tn.Collapse();
                    else if (!tn.IsExpanded && pg.IsExpanded)
                        tn.Expand();
                }
        ...
        private void UpdateColumnSortingIcons()
        {
            ...
            if (m_pListSorter.Column < 0) {
                Debug.Assert(m_lvEntries.ListViewItemSorter == null);
            }
            ...
            bool bForceShowExpired, bool bRespectEntrySearchingDisabled, ref PwGroup pg, bool fParseBrowser,  bool fFinalSearch = true)
        {
            ...
            // only update UI on final search
            if (fFinalSearch) {
    ...
    string strGroupName, bool bForceShowExpired, bool bRespectEntrySearchingDisabled, ref PwGroup pg, bool fParseBrowser);
        ...
        {
            if (!bShowExpired && !bShowSoonToExpire)
                return;

            PwDatabase pd = m_docMgr.ActiveDatabase;
            // https://sourceforge.net/p/keepass/bugs/1150/
            if (!pd.IsOpen || (pd.RootGroup == null))
                return;
        ...
        {
            Debug.Assert(m_docMgr.ActiveDatabase.IsOpen);
            if (!m_docMgr.ActiveDatabase.IsOpen)
                return;
        ...
        private void MoveSelectedEntries(int iMove)
        {
            ...
            Debug.Assert(vEntries != null);
            if (vEntries == null)
                return;
            Debug.Assert(vEntries.Length > 0);
        ...
        {
            if (m_docMgr.ActiveDatabase.IsOpen == false)
                return;
            PwEntry[] vSelected = GetSelectedEntries();
            ...
            PwGroup pgEntrySource, bool bSetModified, Control cOptFocus)
        {
            ...
            if (bUpdateEntryList)
                ...
                Debug.Assert(pgEntrySource == null);
            }
        ...
        private bool PreSaveValidate(PwDatabase pd)
        {
            ...
            if ((pbOnDisk != null) && (pd.HashOfFileOnDisk != null) &&
                ...
                if (dr == DialogResult.Yes) // Synchronize
                {
                    ...
                    Debug.Assert(dr == DialogResult.No);
                }
            }
        ...
        private bool HandleMainWindowKeyMessage(KeyEventArgs e, bool bDown)
        {
            ...
            if (e.Control && !e.Alt) {
                if (e.KeyCode == Keys.Tab) {
                    if (bDown)
                        ActivateNextDocumentEx(e.Shift ? -1 : 1);
                    bHandled = true;
                }
            ...
            PwDatabase pdContext, ref bool bGroupListUpdateRequired)
        {
            ...
            if (pgRecycleBin == null) {
                ...
                Debug.Assert(pgRecycleBin.Uuid.Equals(pdContext.RecycleBinUuid));
            }
        }
        ...
        private static AceColumn GetAceColumn(int nColID)
        {
            ...
            if ((nColID < 0) || (nColID >= v.Count)) {
                Debug.Assert(false);
                return new AceColumn();
            }
        ...
        private void AddOrRemoveTagsToFromSelectedEntries(string strTags, bool bAdd)
        {
            ...
            foreach (PwEntry pe in vModified) {
                pe.Touch(true, false);
            }
